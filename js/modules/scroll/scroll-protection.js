...
export function initScrollProtection() {
  ...
  // scroll-protection нужна всем touch-устройствам включая планшеты
  const isMobile = isMobileDevice({ excludeTablet: false });
  ...
  const handleScroll = () => {
    ...
    if (!isHandlingJump && isUserScrolling && currentPosition < lastScrollPosition - 100 && currentPosition < 100) {
      ...
      isHandlingJump = true;
      // console.warn('Scroll jump detected, restoring position', { from: lastScrollPosition, to: currentPosition }); // DEBUG: отключено
  ...
  window.scrollTo = function(...args) {
    ...
    if (targetY === 0 && currentPosition === 0) {
      ...
      if (now - lastScrollToTime < 100 && scrollToCallCount > 3) {
        ...
        // console.warn('Prevented multiple scrollTo(0) calls', { count: scrollToCallCount }); // DEBUG: отключено
        return;
      }
    ...
    if (args.length > 0 && !isInitialLoad) {
      if (targetY === 0 || (typeof targetY === 'number' && targetY < 50)) {
        ...
        if (isUserScrolling && currentPosition > 100) {
          // console.warn('Prevented scroll reset to top', { currentPosition, targetY }); // DEBUG: отключено
          return; // Не выполняем сброс
        }
        // Также проверяем, не было ли недавнего скролла
        if (currentPosition > 100 && Date.now() - lastScrollTime < 1000) {
          // console.warn('Prevented scroll reset - recent user scroll detected'); // DEBUG: отключено
          return;
        }
      }
    }
  ...
  // console.log('Scroll protection initialized for mobile devices'); // DEBUG: отключено
}
