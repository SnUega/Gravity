...
export function initAccordions() {
  ...
  if (!accordionHeaders.length) {
    // console.log('No accordions found'); // DEBUG: отключено
    return;
  }
  ...
  // console.log(`✅ Accordions initialized: ${accordionHeaders.length} main, ${nestedHeaders.length} nested`); // DEBUG: отключено
}
...
/**
 * Обновить ScrollFlow после изменения аккордеона
 * @param {number} delay - Задержка перед обновлением (по умолчанию 150ms)
 */
function refreshScrollFlow(delay = 150) {
  // Импортируем динамически, чтобы избежать циклических зависимостей
  if (typeof window !== 'undefined' && window.scrollFlowInstance) {
    if (window.scrollFlowInstance.refresh) {
      // Используем debounce для избежания множественных обновлений
      if (!window._scrollFlowRefreshTimeout) {
        window._scrollFlowRefreshTimeout = setTimeout(() => {
          window.scrollFlowInstance.refresh();
          window._scrollFlowRefreshTimeout = null;
        }, delay);
      }
    }
  } else if (true /* ScrollTrigger imported */) {
    // Если ScrollFlow не доступен, обновляем ScrollTrigger напрямую
    if (!window._scrollTriggerRefreshTimeout) {
      window._scrollTriggerRefreshTimeout = setTimeout(() => {
        ScrollTrigger.refresh();
        window._scrollTriggerRefreshTimeout = null;
      }, delay);
    }
  }
}
...
function toggleAccordion(header, content, open, skipScroll = false) {
  ...
  if (open) {
    ...
    setTimeout(() => {
      content.style.maxHeight = content.scrollHeight + 'px';
      // Обновляем ScrollFlow после изменения высоты
      refreshScrollFlow();
    }, 100);
    ...
    const resizeObserver = new ResizeObserver(() => {
      if (header.getAttribute('aria-expanded') === 'true') {
        ...
        requestAnimationFrame(() => {
          content.style.maxHeight = content.scrollHeight + 'px';
          // Обновляем ScrollFlow при изменении размера
          refreshScrollFlow();
        });
      }
    });
    ...
    nestedContents.forEach(nestedContent => {
      const nestedResizeObserver = new ResizeObserver(() => {
        if (header.getAttribute('aria-expanded') === 'true') {
          ...
          requestAnimationFrame(() => {
            content.style.maxHeight = content.scrollHeight + 'px';
            // Обновляем ScrollFlow при изменении размера вложенного аккордеона
            refreshScrollFlow();
          });
        }
      });
    ...
    // Обновляем ScrollFlow после закрытия аккордеона
    // Используем большую задержку для сворачивания, чтобы дать время завершиться анимации
    refreshScrollFlow(350); // Увеличена задержка для сворачивания
  ...
  if (open) {
    ...
    const syncParentHeight = () => {
      ...
      if (elapsed < duration && currentNestedHeight < targetHeight - 1) {
        ...
        delete content._animationFrameId;
        // Обновляем ScrollFlow после завершения анимации вложенного аккордеона
        refreshScrollFlow();
      }
    };
    ...
    const syncParentHeight = () => {
      ...
      if (elapsed < duration && nestedHeight > 1) {
        ...
        parentAccordionContent.style.maxHeight = parentAccordionContent.scrollHeight + 'px';
        delete content._animationFrameId;
        // Обновляем ScrollFlow после завершения анимации закрытия вложенного аккордеона
        refreshScrollFlow();
      }
    };
