...
export class ContactForm {
  ...
  /**
   * Показать предупреждение о политике с анимацией
   */
  _showWarning() {
    if (!this.warning) return;
    this.warning.style.display = 'flex';
    // rAF чтобы браузер успел применить display перед transition
    requestAnimationFrame(() => {
      this.warning.style.opacity = '1';
      this.warning.style.transform = 'translateY(0)';
    });
    // Подписываемся на checkbox один раз — автоскрытие при клике
    if (!this._warningBound) {
      this._warningBound = true;
      this.checkbox.addEventListener('change', () => {
        if (this.checkbox.checked) this._hideWarning();
      }, { once: false });
    }
  }

  /**
   * Скрыть предупреждение с анимацией
   */
  _hideWarning() {
    if (!this.warning) return;
    this.warning.style.opacity = '0';
    this.warning.style.transform = 'translateY(-4px)';
    setTimeout(() => {
