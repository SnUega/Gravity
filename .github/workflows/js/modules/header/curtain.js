...
export class CurtainMode {
  ...
  open() {
    ...
    // На блоге не скрываем хедер при скролле в меню — сбрасываем состояние
    if (this.elements.body && this.elements.body.classList.contains('blog-page')) {
      this.headerHiddenByScroll = false;
      gsap.set(this.elements.header, { y: 0, autoAlpha: 1 });
    }
  ...
  close() {
    ...
    // На странице блога на мобильных сбрасываем возможный сдвиг хедера по Y
    if (this.elements.body && this.elements.body.classList.contains('blog-page')) {
      gsap.set(this.elements.header, { y: 0, autoAlpha: 1 });
    }
  ...
  setupScrollHandlers() {
    ...
    const scrollHandler = () => {
      ...
      // На странице блога хедер не скрываем при скролле в меню
      if (this.elements.body && this.elements.body.classList.contains('blog-page')) {
