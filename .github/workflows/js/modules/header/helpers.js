...
export function animateHeaderContent(elements, visible, duration = 0.2) {
  if (!elements.headerLogo && !elements.headerContact && !elements.bookingBtn) return;
  try {
    const elementsToAnimate = [elements.headerLogo, elements.headerContact, elements.bookingBtn].filter(Boolean);
    ...
    // console.warn('Failed to animate header content:', error); // DEBUG: отключено
  }
}
...
export function setPanelClipPath(menuPanel, panelW, currentW) {
  ...
  try {
    ...
    // console.warn('Failed to set panel clip path:', error); // DEBUG: отключено
  }
}
...
export function getCurrentStemWidth(menuStem, defaultWidth = 64) {
  ...
  try {
    ...
    // console.warn('Failed to get stem width:', error); // DEBUG: отключено
    return defaultWidth;
  }
}
