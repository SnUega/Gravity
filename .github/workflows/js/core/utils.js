...
/**
 * Проверка, является ли устройство мобильным (по UA + touch + ширине экрана).
 * Планшеты (768–1024px) по умолчанию НЕ считаются мобильными —
 * это важно для Lenis и других модулей где планшет получает десктопное поведение.
 *
 * @param {Object} options
 * @param {boolean} options.excludeTablet — исключить планшеты (default: true)
 * @returns {boolean}
 */
export function isMobileDevice({ excludeTablet = true } = {}) {
  const ua    = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent);
  const touch = ('ontouchstart' in window) || (navigator.maxTouchPoints > 0);
  const narrow = window.innerWidth < 768;
  if (excludeTablet && window.innerWidth >= 768 && window.innerWidth <= 1024) return false;
  return (ua || touch) && narrow;
}

/**
 * Проверка, является ли устройство мобильным (только по ширине экрана)
 * @returns {boolean}
 */
